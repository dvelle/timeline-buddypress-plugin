...
<?php
	...
	function wpvt_post_type_init() {
		...
		$labels = array(
			'name' => _x('FanBook Entries', 'post type general name'),
			'singular_name' => _x('FanBook Entry', 'post type singular name'),
			'add_new' => _x('Add New', 'FanBook'),
			'add_new_item' => __('Add New FanBook Entry'),
			'edit_item' => __('Edit FanBook Entry'),
			'new_item' => __('New FanBook Entry'),
			'all_items' => __('All FanBook Entries'),
			'view_item' => __('View FanBook Entry'),
			'search_items' => __('Search FanBook Entries'),
			'not_found' =>  __('No FanBook Entries found'),
			'not_found_in_trash' => __('No FanBook Entries found in Trash'), 
			'parent_item_colon' => '',
			'menu_name' => __('FanBook')
		);
		$args = array(
			'labels' => $labels,
			'public' => true,
		...
		register_post_type( 'fanbook' , $args );
		
		$labels = array(
			'name' => _x('GroupBook Entries', 'post type general name'),
			'singular_name' => _x('GroupBook Entry', 'post type singular name'),
			'add_new' => _x('Add New', 'GroupBook'),
			'add_new_item' => __('Add New GroupBook Entry'),
			'edit_item' => __('Edit GroupBook Entry'),
			'new_item' => __('New GroupBook Entry'),
			'all_items' => __('All GroupBook Entries'),
			'view_item' => __('View GroupBook Entry'),
			'search_items' => __('Search GroupBook Entries'),
			'not_found' =>  __('No GroupBook Entries found'),
			'not_found_in_trash' => __('No GroupBook Entries found in Trash'), 
			'parent_item_colon' => '',
			'menu_name' => __('GroupBook')
		);
		$args = array(
			'labels' => $labels,
			'public' => true,
		...
		register_post_type( 'groupbook' , $args );
...
/* Metaboxes for Fanbook Post Type */
	function wpvt_FB_meta_boxes() {
		add_meta_box( 'fanbook-meta', 'FanBook Meta Data', 'wpvt_fb_meta_boxes_inner', 'fanbook' );
	}
	add_action( 'add_meta_boxes', 'wpvt_FB_meta_boxes' );

	/* Prints the box content */
	function wpvt_fb_meta_boxes_inner() {
		global $post;
		wp_nonce_field( plugin_basename( __FILE__ ), 'wpvt_noncename' ); // Use nonce for verification
		...
		<div class="wpvt-metabox">
			...
			<input type="submit" class="button" name="wpvt_meta_submit" value="Save FanBook Data" />
		</div>
		<?php
	}
	...
	function insertarecord($user_ID,$string,$type,$book_id=false,$group_id=false)
	{
		...
		if($type=='scrapbook')
		{
			...
			//test if new or update
			$previous_fb = $wpdb->get_var($wpdb->prepare("SELECT cover_post_id FROM {$wpdb->prefix}ltpmy_fanbooks WHERE id = '".$book_id."'"));
			if($previous_fb)
			{
				//update
			...
			//test if new or update
			$previous_gb = $wpdb->get_var($wpdb->prepare("SELECT id FROM {$wpdb->prefix}ltpmy_groupbooks  WHERE group_id = '".$group_id."' AND cover_post_id = '".$cover_post_id."'"));
			if(!empty($previous_gb))
			{
				//update
				$wpdb->get_results("UPDATE {$wpdb->prefix}ltpmy_groupbooks SET json_string='$string' WHERE group_id='".$group_id."' AND cover_post_id = '".$cover_post_id."'");
			} else {
				//insert
				$wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}ltpmy_groupbooks (group_id,admin_id,cover_post_id,json_string)
									VALUES(%d, %d, %d, %s)",$group_id, $user_ID, $cover_post_id, $string));
			}
		}
	}
	function getBookCover($author_id,$type,$book_id=false)
	{
		global $wpdb;
		//work with database
		if($type=='scrapbook')
		{
			//test if new or update
			$cover_post_id = $wpdb->get_var($wpdb->prepare("SELECT cover_post_id FROM {$wpdb->prefix}ltpmy_scrapbooks  WHERE uid = '".$author_id."'"));			
		} elseif($type=='fanbook'){
			//test if new or update
			$cover_post_id = $wpdb->get_var($wpdb->prepare("SELECT cover_post_id FROM {$wpdb->prefix}ltpmy_fanbooks  WHERE id = '".$book_id."'"));
		} elseif($type=='groupbook'){
			//test if new or update
			$cover_post_id = $wpdb->get_var($wpdb->prepare("SELECT cover_post_id FROM {$wpdb->prefix}ltpmy_groupbooks  WHERE group_id = '".$group_id."' AND id = '".$book_id."'"));
		}
		return $cover_post_id;
	}
	/* Save JSON file */
	function wpvt_update_json( $post_id, $type,$book_id=false) 
	{
		global $post;
global $wpdb;
		$post = get_post( $post_id );		
			...
			$cover_post_id = getBookCover($author_id,$type,$book_id);
			if($cover_post_id)
			{
			...
			if($type=='scrapbook'){
				$args = array( 'post_type' => $type ,'author' => $author_id );
				$loop = new WP_Query( $args );
			}elseif($type=='fanbook'){
				$parent_id = $wpdb->get_var($wpdb->prepare("SELECT managing_post_id FROM {$wpdb->prefix}ltpmy_fanbooks  WHERE id = '".$book_id."'"));
				$loop = new WP_Query( 'post_parent=$parent_id' );
			}
	...
	function wpvt_sc_func($atts)
	{
		...
		{			
			...
			if($type=='scrapbook')
			{
				...
				//test if new or update
				$users_file = $wpdb->get_var($wpdb->prepare("SELECT json_string FROM {$wpdb->prefix}ltpmy_groupbooks  WHERE group_id = '".$group_id."' AND id = '".$book_id."'"));
			}
			...
			{		
				...
				$s = $wpdb->get_results("SELECT start,font,language FROM {$wpdb->prefix}ltpmy_sb_settings WHERE uid = '".$author_id."' AND book_id='".$book_id."' ");
				if(empty($s))
				{
