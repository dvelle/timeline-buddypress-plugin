...
<?php
	...
	function wpvt_update_json( $post_id ) {
		...
		if($post->post_type == 'scrapbook') 		{						
			//Author Info			
			$author_id = $post->post_author;			
			$author_info = get_userdata($author_id);			
			$nicename = $author_info->user_login;			
			$options = get_option('wpvt_options');
			...
			{
				$who = $new_post->post_author;
				if($who == $author_id)
				{
					$cover_post_id  = $new_post->ID;
				}
			}
			...
			$args = array( 'post_type' => 'scrapbook','author' => $author_id );
			...
			';			
			$users_file = "$nicename.jsonp";			
			$jsonFile = plugin_dir_path( __FILE__ ) . "/". $users_file;
	...
	function wpvt_sc_func($atts)
	{
		global $post;	
		global $wpdb;
		extract(shortcode_atts(array("author_id" => 0), $atts));
		if($author_id != 0)
		{			
			...
			$filename = ABSPATH."wp-content/plugins/ltpmy-scrapbook/".$users_file;
			clearstatcache();
			...
			{		
				...
				$s = $wpdb->get_results("SELECT start,font,language FROM {$wpdb->prefix}ltpmy_sb_settings WHERE uid = '".$author_id."'");
				if(empty($s))
				{
				...
				echo '
					...
					var timeline_config = {
						...
						source: "'.plugins_url( $users_file, __FILE__ ).'",
